...
use Nsulistiyawan\Bpjs\BpjsPcare;

class Referensipcare extends BpjsPcare
{
    public function diagnosa($keyword,$start,$limit)
    {
        $record1 = !empty($start) ? '/'.$start : '';
        $record2 = !empty($limit) ? '/'.$limit : '';
        $response = $this->get('diagnosa/'.$keyword . $record1 . $record2);
